...
def preprocess_floor_plan(room_data_dir, room_side, num_sampled_points=256, overwrite=False):
    ...
    for e in os.listdir(room_data_dir): # train, val, and test
        ...
        scene_data = np.load(os.path.join(room_data_dir, e, "cuboid_scene.npz"), allow_pickle=True)
        ...
        if overwrite:
            print(f"Saving floor plan data to {os.path.join(room_data_dir, e, 'cuboid_scene.npz')}")
            np.savez_compressed(os.path.join(room_data_dir, e, "cuboid_scene.npz"),  **scene_data)
